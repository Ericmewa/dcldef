...
const DeferralDetailsModal = ({
  ...
  const resolveDocDaysAndDate = (doc, deferral) => {
    ...
    // New: alternative resolver (matches DeferralPending) that returns { days, nextDate }
    const resolveDocDaysAndDate2 = (doc, deferral) => {
      ...
      if (!nextDate) {
        const fallback =
          ...
          deferral?.createdAt;
        const parsed = dayjs(fallback);
        if (parsed.isValid()) nextDate = parsed.toISOString();
      }
                      ...
                      >
                        ...
                        >
                          ...
                          <div>
                            ...
                            {(() => {
                              const resolved = resolveDocDaysAndDate2(
                                doc,
                                deferral,
                              );
                              return (
                                ...
                                >
                                  ...
                                  <div>
                                    <b>New due date:</b>{" "}
                                    {resolved.nextDate
                                      ? dayjs(resolved.nextDate).format(
                                          "DD MMM YYYY",
                                        )
                                      : "-"}
                                  </div>
...
const MyQueue = () => {
  ...
  useEffect(() => {
    ...
    const handleExtensionUpdated = (event) => {
      const updatedExtension = event.detail;
      if (!updatedExtension) return;
      setDeferrals((prev) =>
        prev.map((d) => {
          if (d && d.isExtension && d.extension) {
            const extId = d.extension._id || d.extension.id;
            const updatedId = updatedExtension._id || updatedExtension.id;
            if (extId && updatedId && String(extId) === String(updatedId)) {
              return {
                ...d,
                extension: updatedExtension,
                status: updatedExtension.status || d.status,
                approvers: updatedExtension.approvers || d.approvers,
              };
            }
          }
          return d;
    ...
    const handleExtensionCreated = (event) => {
      const payload = event.detail;
      const createdExtension = payload?.extension || payload;
      const deferralBase = payload?.deferral || payload?.deferral || null;
      if (!createdExtension) return;

      // Add to queueExtensions if not already present
      setQueueExtensions((prev) => {
        const exists = prev.some(
          (e) => (e._id || e.id) === (createdExtension._id || createdExtension.id),
        );
        if (exists) return prev;
        return [createdExtension, ...(prev || [])];
      });

      // Also merge into deferrals list so the main table reflects it
      setDeferrals((prev) => {
        const mapped = (prev || []).slice();
        const base = deferralBase || createdExtension.deferral || {};
        const baseId = base._id || base.id || createdExtension.deferralId || createdExtension.deferralId || null;
        const id = baseId || createdExtension._id || createdExtension.id || `ext-${Math.random().toString(36).slice(2)}`;
        const entry = {
          ...base,
          _id: id,
          id,
          ...
          approvers: createdExtension.approvers || base.approvers || [],
          approverFlow: createdExtension.approverFlow || base.approverFlow || [],
          status: createdExtension.status || base.status || "pending_approval",
        };
        // prepend
        return [entry, ...mapped];
      });
  ...
  const fetchDeferrals = async () => {
    setIsLoading(true);
    try {
      // Fetch approver deferrals and pending extensions in parallel
      const [queue, extQueue] = await Promise.all([
        deferralApi.getApproverQueue(token),
        deferralApi.getApproverExtensionQueue(token).catch(() => []),
      ]);

      // Map extensions into deferral-like entries so they appear in the same list
      const mappedExtensions = (extQueue || []).map((ext) => {
        const base = ext.deferral || {};
        const baseId = base._id || base.id || ext.deferralId || ext.deferralId || null;
        const id = baseId || ext._id || ext.id || `ext-${Math.random().toString(36).slice(2)}`;
        return {
          ...base,
          _id: id,
          id,
          isExtension: true,
          extension: ext,
          approvers: ext.approvers || base.approvers || [],
          approverFlow: ext.approverFlow || base.approverFlow || [],
          status: ext.status || base.status || "pending_approval",
        };
      });

      setDeferrals([...(queue || []), ...mappedExtensions]);
    } catch (error) {
