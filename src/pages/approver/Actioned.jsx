...
const Actioned = () => {
  ...
  const resolveDocDaysAndDate = (doc, deferral) => {
    ...
    if (
      ...
      const match = deferral.selectedDocuments.find((sd) => {
        if (!sd) return false;
        const sdName = (
          sd.name || sd.label || (typeof sd === "string" ? sd : "")
        )
          .toString()
          .toLowerCase();
  ...
  const downloadDeferralAsPDF = async () => {
    ...
    try {
      ...
      loanItems.push({ label: "Days Sought", value: `${selected.daysSought || 0} days` });
      loanItems.push({
                      ...
                      >
                        ...
                        >
                          ...
                          <div>
                            ...
                            {(() => {
                              const resolved = resolveDocDaysAndDate(doc, selected);
                              return (
                                <div
                                  style={{
                                    marginTop: 6,
                                ...
                                >
                                  ...
                                  <div>
                                    <b>New due date:</b>{" "}
                                    {resolved.date
                                      ? dayjs(resolved.date).format("DD MMM YYYY")
                                      : "-"}
                                  </div>
                                </div>
                              );
