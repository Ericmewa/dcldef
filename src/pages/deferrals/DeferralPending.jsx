            ...
            onClick={async () => {
              ...
              setExtensionSubmitting(true);
                try {
                  ...
                  // Submit extension and capture returned extension info
                  const created = await deferralApi.submitExtension(
                  ...
                  // Immediately update local state so UI reflects the new extension
                  // created may contain { extension: { id, deferralId, status, requestedDaysSought, ... } }
                  const extensionObj = created?.extension || created;

                  // Update deferrals list
                  setDeferrals((prev) => {
                    return prev.map((d) => {
                      const id = d._id || d.id;
                      const selId = selectedDeferralForExtension._id || selectedDeferralForExtension.id;
                      if (String(id) === String(selId)) {
                        const next = { ...(d || {}) };
                        next.extensionStatus = extensionObj?.status || "PendingApproval";
                        next.extensions = next.extensions ? [...next.extensions, extensionObj] : [extensionObj];
                        return next;
                      }
                      return d;
                    });
                  ...
                  // Update selected deferral displayed in modal/list
                  setSelectedDeferral((prev) => {
                    if (!prev) return prev;
                    const prevId = prev._id || prev.id;
                    const selId = selectedDeferralForExtension._id || selectedDeferralForExtension.id;
                    if (String(prevId) === String(selId)) {
                      return {
                        ...prev,
                        extensionStatus: extensionObj?.status || "PendingApproval",
                        extensions: prev.extensions ? [...prev.extensions, extensionObj] : [extensionObj],
                      };
                    }
                    return prev;
                  });

                  // Dispatch update events so approver queues and other pages pick it up
                  try {
                    const updatedDef = {
                    ...
                    // Inform deferral-aware components
                    window.dispatchEvent(
                      new CustomEvent("deferral:updated", { detail: updatedDef }),
                    );

                    // Inform approver-facing components about the new extension
                    const createdPayload = {
                      extension: extensionObj,
                      deferral: selectedDeferralForExtension,
                    };
                    window.dispatchEvent(
                      new CustomEvent("extension:created", { detail: createdPayload }),
                    );

                    // Also broadcast an extension:updated event for components expecting it
                    window.dispatchEvent(
                      new CustomEvent("extension:updated", { detail: extensionObj }),
                    );
                  } catch (e) {
                    console.debug("Failed to dispatch extension events", e);
                  }

                  // Close extension modal but keep main modal open so RM can see status
                  setExtensionModalOpen(false);
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  >
                    {requestedDocs.map((doc, idx) => {
                      // resolve current due date (inline to avoid cross-scope reference)
                      const tryValues = (obj, keys) => {
                        for (const k of keys) {
                          if (typeof obj[k] !== "undefined" && obj[k] !== null) return obj[k];
                        }
                      ...
                      let resolvedDays = tryValues(doc, dayKeys);
                      let resolvedNextDate = tryValues(doc, dateKeys);
                      // try matching selectedDocuments from the deferral
                      if ((typeof resolvedDays === "undefined" || typeof resolvedNextDate === "undefined") && Array.isArray(selectedDeferralForExtension?.selectedDocuments)) {
                        const docName = (doc && (doc.name || doc.label)) || String(doc || "");
                        const match = selectedDeferralForExtension.selectedDocuments.find((sd) => {
                          const sdName = (sd && (sd.name || sd.label)) || String(sd || "");
                          return sdName && sdName.toLowerCase().trim() === String(docName).toLowerCase().trim();
                        });
                        if (match) {
                          if (typeof resolvedDays === "undefined") resolvedDays = tryValues(match, dayKeys);
                          if (typeof resolvedNextDate === "undefined") resolvedNextDate = tryValues(match, dateKeys);
                        }
                      }
                      if (!resolvedNextDate) {
                        const fallback = selectedDeferralForExtension?.nextDueDate || selectedDeferralForExtension?.nextDocumentDueDate || selectedDeferralForExtension?.next_due_date || selectedDeferralForExtension?.next_document_due_date || selectedDeferralForExtension?.createdAt;
                        const parsed = dayjs(fallback);
                        if (parsed.isValid()) resolvedNextDate = parsed.toISOString();
                      }
                      const currentDueDate = resolvedNextDate ? dayjs(resolvedNextDate) : null;
                      const key = String(
                        (doc && (doc.name || doc.label)) || doc || "",
                      )
                        .trim()
